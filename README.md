...
First run downloads ~18 GB of model weights, so give it a while. When you see `Running on local URL: http://127.0.0.1:7860`, open that address in your browser. Model loading itself takes a few quiet minutes — don't ctrl-C.
...
```
--model <hf_repo>   Model to use (default: aoi-ot/VibeVoice-Large)
                    Smaller/faster: --model microsoft/VibeVoice-1.5B
